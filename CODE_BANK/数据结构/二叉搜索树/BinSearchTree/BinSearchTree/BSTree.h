...
public:
	int Sum(BSTNode<Key, E>* root, int level) const;
	BSTree() { root = NULL; nodecount = 0; } // Constructor
	~BSTree() { clearhelp(root); } // Destructor
...
template <typename Key, typename E>
BSTNode<Key, E>* BSTree<Key, E>::inserthelp(
...
template <typename Key, typename E>
E BSTree<Key, E>::findhelp(BSTNode<Key, E>* root,
	const Key& k) const {
	...
	template <typename Key, typename E>  
	int BSTree<Key, E>::Sum(BSTNode<Key, E>* root,int level) const{  
	    static int sum=0;  
